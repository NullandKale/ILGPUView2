...
public class PlyData
{
    ...
    public static PlyData Load(string filePath)
    {
        ...
        using (BinaryReader br = new BinaryReader(fs))
        {
            ...
            stopwatch.Stop();
            Console.WriteLine($"ReadHeader Time: {stopwatch.ElapsedMilliseconds} ms");

            stopwatch.Restart();
            plyData.ReadData(br);
            stopwatch.Stop();
            Console.WriteLine($"ReadData Time: {stopwatch.ElapsedMilliseconds} ms");
        }
        return plyData;
    }
    ...
    public T GetProperty<T>(int index, string propertyName)
    {
        ...
        return (T)Convert.ChangeType(BitConverter.ToSingle(dataBuffer, dataOffset), typeof(T));
    }
